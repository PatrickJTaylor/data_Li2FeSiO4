...
def main() -> None:
    ...
    for idx, temperature in enumerate(temperatures):
        for frame_idx in frame_sets[idx]:
            ...
            fe_magmoms = magmoms[:n_fe]
            for oxidation_state, magmom in zip(fe_oxidation_states, fe_magmoms):
                abs_magmom = abs(magmom)

                abs_fe_magmoms[oxidation_state].append(abs_magmom)
    ...
    for baseline_idx, oxidation_state in enumerate(abs_fe_magmoms):
        abs_magmoms = abs_fe_magmoms[oxidation_state]
        y_scatter = (
            np.full_like(abs_magmoms, baseline_idx) * 2
            + np.random.random(size=len(abs_magmoms)) * 0.15
        )

        kde = gaussian_kde(abs_magmoms, bandwidth / np.std(abs_magmoms))
        ...
        percentiles = np.percentile(abs_magmoms, [0, 25, 50, 75, 100])
    ...
    ax.set_xlabel(r"absolute Fe magnetic moment / $\mu_{\mathrm{B}}$")
    ...
    ax.annotate(r"Fe$^{\mathrm{III}}$", (0.77, 0.21), xycoords="axes fraction")
    ax.annotate(r"Fe$^{\mathrm{IV}}$", (0.56, 0.53), xycoords="axes fraction")
    ax.annotate(r"Fe$^{\mathrm{V}}$", (0.31, 0.87), xycoords="axes fraction")
